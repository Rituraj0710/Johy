...
class AgentController {
  ...
  static agentRegistration = async (req,res) =>{
    try {
      ...
      const newUser = await UserModel.create({name, phone, email, password:hashedPassword, roles:["agent"]});
      await newUser.save();
  ...
  static agentLogin = async (req, res) => {
    try {
      ...
      if(!user.roles.includes("agent")){
        return res.status(403).json({status: "failed", message: "Access denied. Not an agent"})
      }
  ...
  static agentProfile = async(req,res) => {
    res.send({"user": req.user})
  }

  // Change Password
  static changeAgentPassword = async (req,res) =>{
    try {
  ...
  static sendAgentPasswordResetEmail = async (req, res) =>{
    try {
  ...
  // Password Reset
  static agentPasswordReset = async (req,res) =>{
    try {
      const {password, password_confirmation} = req.body;
  ...
  // Logout
  static agentLogout = async (req,res) =>{
    try {

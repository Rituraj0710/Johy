...
export const createPropertySaleCertificate = async (req, res) => {
  try {
    const data = req.body;
    data.purchasers = JSON.parse(data.purchasers || '[]');
    data.payments = JSON.parse(data.payments || '[]');
    data.witnesses = JSON.parse(data.witnesses || '[]');
    data.power_authority = JSON.parse(data.power_authority || '[]');
    data.floors = JSON.parse(data.floors || '[]'); // If applicable
    ...
    if (fileMap['bank_rep_photo']) data.bank_rep_photo = fileMap['bank_rep_photo'];
    if (fileMap['prop_photo']) data.prop_photo = fileMap['prop_photo'];

    // Purchaser photos
    const purchaserPhotoFields = Object.keys(fileMap).filter(key => key.startsWith('purchaser_photo_'));
    purchaserPhotoFields.sort((a, b) => parseInt(a.split('_')[2]) - parseInt(b.split('_')[2]));
    purchaserPhotoFields.forEach((field, i) => {
      if (data.purchasers[i]) data.purchasers[i].photo = fileMap[field];
    });

    // Witness photos
    const witnessPhotoFields = Object.keys(fileMap).filter(key => key.startsWith('witness_photo_'));
    witnessPhotoFields.sort((a, b) => parseInt(a.split('_')[2]) - parseInt(b.split('_')[2]));
    witnessPhotoFields.forEach((field, i) => {
      if (data.witnesses[i]) data.witnesses[i].photo = fileMap[field];
    });

    const newCert = new PropertySaleCertificate(data);
    await newCert.save();

    res.status(201).json({ status: 'success', message: 'Property Sale Certificate created', id: newCert._id });
  } catch (error) {
...
export const getPropertySaleCertificate = async (req, res) => {
  try {
    const cert = await PropertySaleCertificate.findById(req.params.id);
    if (!cert) return res.status(404).json({ status: 'fail', message: 'Not found' });
...
export const getAllPropertySaleCertificates = async (req, res) => {
  try {
    const certs = await PropertySaleCertificate.find({});

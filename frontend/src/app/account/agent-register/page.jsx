...
import { useCreateAgentMutation } from '@/lib/services/auth';
const initialValues = {
  name: "",
  phone: "",
  email: "",
...
const Register = () => {
  ...
  const [createUser] = useCreateAgentMutation()
    ...
    onSubmit: async(values, action) => {
     ...
     try {
       ...
       if(response.data && response.data.status === 'success'){
        ...
        action.resetForm();
        setLoading(false);
        router.push('/account/agent-verify-email')
       }
  ...
  return (
    <div className='flex items-center justify-center h-screen bg-slate-400'>
      <div className='w-full max-w-md p-8 bg-white rounded-lg shadow-lg'>
        <h2 className='text-2xl font-bold mb-6 text-center'>Agent Register</h2>
        ...
        <p className='text-sm text-gray-600 p-1'>Already an User? <Link href="/account/agent-login" className='text-indigo-500 hover:text-indigo-600 transition duration-300 ease-in-out '>Login</Link> </p>
